...
namespace Sound
{
    public class EnemySpawner : Sounds
    {
        ...
        void Start()
        {
            foreach (var obj in enemyPrefabs)
            {
                GameManager.Instance.poolManager.InitiatePool(
                    obj, 100,
                    obj.GetComponent<Margot.Enemy>().enemyType.ToString() + "Enemy"
                );
            }
        }
        ...
        public GameObject SpawnFromPool(string poolKey)
        {
            ...
            var e = enemy.GetComponent<Margot.Enemy>();
            if (e != null)
            {
                e.UpdateStat();
        ...
        /// </summary>
        private IEnumerator SpawnWarmupRoutine(GameObject enemyGO, Margot.Enemy enemyComp, float seconds)
        {
